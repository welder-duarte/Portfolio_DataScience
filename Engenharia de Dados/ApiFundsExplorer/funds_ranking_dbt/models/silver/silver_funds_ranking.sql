{{ config(
    ...
    unique_key=['fundos', 'scraping_date']
) }}
...
WITH base AS(
    ...
    SELECT
        ...
        SAFE_CAST(REPLACE(variacao_cotacao_mes, ',', '.') AS FLOAT64)       AS variacao_preco_m,
        SAFE_CAST(REPLACE(volatility, ',', '.') AS FLOAT64)                 AS volatilidade,
        ...
        SAFE_CAST(REPLACE(yield_vpa_3m_sum, ',', '.') AS FLOAT64)           AS yield_vpa_3m_acum,
        SAFE_CAST(REPLACE(yield_vpa_3m, ',', '.') AS FLOAT64)               AS yield_vpa_3m,

        SAFE_CAST(REPLACE(yield_vpa_6m_sum, ',', '.') AS FLOAT64)           AS yield_vpa_6m_acum,
        SAFE_CAST(REPLACE(yield_vpa_6m, ',', '.') AS FLOAT64)               AS yield_vpa_6m,

        SAFE_CAST(REPLACE(yield_vpa_12m_sum, ',', '.') AS FLOAT64)          AS yield_vpa_12m_acum,
    ...
    FROM {{ref('raw_funds_ranking')}}

    {% if is_incremental() %}
        WHERE scraping_date > (SELECT max(scraping_date) FROM {{ this }})
    {% endif %}
)
